...
public interface WarehouseHistoryRepository extends JpaRepository<WarehouseHistory, Integer> {
    ...
    Optional<List<WarehouseHistory>> getByWarehouseId(int warehouseId);

    @Query(value = WarehouseHistoryQuery.getByWarehouseIdDesc, nativeQuery = true)
    Optional<List<WarehouseHistory>> getByWarehouseIdDesc(int warehouseId);
}
